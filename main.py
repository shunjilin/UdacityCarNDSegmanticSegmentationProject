...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    upsample1 = tf.layers.conv2d_transpose(
        layer7_conv, num_classes, 4, strides=(2, 2), padding="same",
        kernel_initializer= tf.random_normal_initializer(stddev=0.01),
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    skip1 = tf.add(upsample1, layer4_conv)

    upsample2 = tf.layers.conv2d_transpose(
        skip1, num_classes, 4, strides=(2, 2), padding="same",
        kernel_initializer= tf.random_normal_initializer(stddev=0.01),
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    skip2 = tf.add(upsample2, layer3_conv)

    upsample3 = tf.layers.conv2d_transpose(
        skip2, num_classes, 16, strides=(8, 8), padding="same",
        kernel_initializer= tf.random_normal_initializer(stddev=0.01),
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
